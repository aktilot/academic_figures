...
```{r default_scatter, fig.height=3, fig.width=5}
scatter_plot <- iris %>%
...
```{r better_scatter, fig.height=3, fig.width=5}
library(cowplot)
better_scatter <- iris %>%
  ggplot(aes(x = Petal.Width, y = Petal.Length, color = Species)) +
  geom_point()
...
```{r better_scatter2, fig.height=3, fig.width=5}
...
```{r box_plot, fig.height=3.5, fig.width=3}
box_plot <- iris %>%
...
```{r better_box, fig.height=3.5, fig.width=3}
better_box <- box_plot +
...
```{r saving}
ggsave(scatter_plot, filename = "initial_scatterplot.svg", width = 6, height = 6, units = "in")
ggsave(scatter_plot, filename = "initial_scatterplot.jpg", width = 6, height = 6, units = "in", dpi = 300)
```
...
ggsave(plot = better_scatter_box, filename = "double_plot.png", device = "png")
```
...
#### Using an non-R-generated image as a figure panel
